...
const Executer = (function() {
  ...
  executer.prototype.VariableDeclaration=function(node){
    for (let dec of node.declarations) {
      if (dec.init) {
        if (dec.init.type == "ObjectExpression") {
          let temp={}
          for(let prop of  dec.init.properties){
            if(prop.value.type="Literal"){
                temp[prop.key]=prop.value.value
            }
            else if(prop.value.type="ObjectExpression")
            {
                temp[prop.key]= this.variableEnvironment(prop.value)
            }
          }
          thie.enviromentManager.addValueToHeap()
